...
{
	suite = CU_add_suite("Ray transformation", NULL, NULL);
	if (suite == NULL)
		return (CU_cleanup_registry(), CU_get_error());

	if (!CU_add_test(suite, "Ray scaling 00", do_ray_scaling_00)
		|| !CU_add_test(suite, "Ray scaling 01", do_ray_scaling_01))
		return (CU_cleanup_registry(), CU_get_error());

	if (!CU_add_test(suite, "Ray translation 00", do_ray_translate_00)
		|| !CU_add_test(suite, "Ray translation 01", do_ray_translate_01)
		|| !CU_add_test(suite, "Ray translation 02", do_ray_translate_02))
		return (CU_cleanup_registry(), CU_get_error());
